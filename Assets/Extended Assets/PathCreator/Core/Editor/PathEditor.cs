...
using Assets.PathCreator.Core.Editor;
        ...
        private void ProcessBezierPathInput(Event e)
        {
            ...
            if (mouseOverHandleIndex == -1)
            {
                if (e.type == EventType.MouseDown && e.button == 0 && e.shift)
                {
                    ...
                    {
                        Vector3 newPathPoint = pathMouseInfo.closestWorldPointToMouse;//Andrix
                    ...
                    else if (!bezierPath.IsClosed)
                    {
                        ...
                        if (true)
                            newPathPoint = Assets.PathCreator.Core.Editor.AndrixExtended.GetPoint();//MouseUtility.GetMouseWorldPosition(bezierPath.Space, dstCamToEndpoint);//Andix
                        else
                            newPathPoint = MouseUtility.GetMouseWorldPosition(bezierPath.Space, dstCamToEndpoint);
